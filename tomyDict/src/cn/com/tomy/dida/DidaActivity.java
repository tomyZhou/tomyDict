...
import cn.com.tomy.utils.MyAdapter;
import cn.com.tomy.utils.SearchWords;
...
public class DidaActivity extends Activity {
	...
	private int maxVolume;// 最大音量0-15

	private static Context context;
	private static final int VOICE_RECOGNITION_REQUEST_CODE = 1234;
	protected static final String TAG = "DidaActivity";
	...
	private Handler handler = new Handler() {
		public void handleMessage(android.os.Message msg) {
			...
			if (dict != null && !"".equals(dict)) {
				//如果没有查到，就把原单位放到text_word的位置上。否则就把查找到内容提要放到text_word的位置上.
				...
				if (ps == null || "".equals(ps)) {
					...
					for(int i=0;i<dict.getSents().size();i++){
						sb.append(dict.getSents().get(i).getOrig()+":"+dict.getSents().get(i).getTrans());
					}
				}
			} else {
				Toast.makeText(getApplicationContext(), "查询失败,请检查网络", 0).show();
			}
		};
	};
		...
		btn_search.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				if (CheckNet()) {
					...
					try {
						final String words = URLEncoder.encode(word, "UTF-8");
						Log.i(TAG, words);
	...
	public static void playAudio(String path) {
		...
		try {
			mp = MediaPlayer.create(context, Uri.parse(path)); // 从网络直接获取声音文件
